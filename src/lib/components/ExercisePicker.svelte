...
<div class="flex flex-wrap gap-1 mb-2">
  {#each categories as cat}
    ...
    <button
      class="py-1.5 px-2.5 rounded-lg text-[0.55rem] font-bold tracking-[1px] uppercase transition-all
...
<!-- Exercise buttons (wrapping grid) -->
<div class="flex flex-wrap gap-2">
  {#each filtered as ex}
    <div class="relative group">
      ...
      <button
        class="relative py-2.5 px-3 rounded-[12px] text-[0.6rem] font-bold tracking-[1.5px] uppercase transition-all -skew-x-3 whitespace-nowrap
          {selected === ex.id
